...
{
	public class GenStep_Trench : GenStep
	{
		private struct NeededTrench
		{
			public float angle;
        ...
        public BuildableDef trench = null;
		public BuildableDef sandBag = null;

		public char attackDir = '\0'; //make this vector of the center tile
		...
		{
			trench = HOIThingDefOf.GW_Trenches;
			sandBag = ThingDefOf.Sandbags;

			List<NeededTrench> neededRoads = CalculateNeededRoads(map);
			...
			RoadDef bestRoadType = DefDatabase<RoadDef>.AllDefs.Where((RoadDef rd) => neededRoads.Count((NeededTrench nr) => nr.road == rd) >= 2).MaxByWithFallback((RoadDef rd) => rd.priority);
			if (bestRoadType != null)
			{
				NeededTrench neededRoad = neededRoads[neededRoads.FindIndex((NeededTrench nr) => nr.road == bestRoadType)];
				neededRoads.RemoveAt(neededRoads.FindIndex((NeededTrench nr) => nr.road == bestRoadType));
				NeededTrench neededRoad2 = neededRoads[neededRoads.FindIndex((NeededTrench nr) => nr.road == bestRoadType)];
				neededRoads.RemoveAt(neededRoads.FindIndex((NeededTrench nr) => nr.road == bestRoadType));
			...
			foreach (NeededTrench item in neededRoads)
			{
		...
		private List<NeededTrench> CalculateNeededRoads(Map map)
		{
			List<int> list = new List<int>();
			Find.WorldGrid.GetTileNeighbors(map.Tile, list);
			List<NeededTrench> list2 = new List<NeededTrench>();
			foreach (int item in list)
			{
				RoadDef roadDef = Find.WorldGrid.GetRoadDef(map.Tile, item);
				if (roadDef != null && roadDef.defName.Equals("TrenchPath"))
				{
					list2.Add(new NeededTrench
					{
			...
			if (list2.Count > 1)
			{
				...
				foreach (NeededTrench item2 in list2)
				{
		...
		private Action PrepDrawRoad(Map map, TerrainDef rockDef, IntVec3 start, IntVec3 end, RoadDef roadDef, RoadPathingDef pathingDef, out IntVec3 centerpoint)
		{
			...
			return delegate
			{
				ApplyDistanceField(distance, map);
			};
		}
		...
		private void ApplyDistanceField(DistanceElement[,] distance, Map map)
		{
            for (int i = 0; i < map.Size.x; i++)
			{
				for (int j = 0; j < map.Size.z; j++)
				{
					...
					if (distanceElement.fromRoad <1)
					{
                        IntVec3 position = new IntVec3(i, 0, j);
                        PlaceTrench(map, position, trench);
                    }
					else if (distanceElement.fromRoad < 1.75 && Rand.Chance(0.6f))
					{
                        IntVec3 position = new IntVec3(i, 0, j);
                        PlaceTrench(map, position, sandBag);
                    }
				}
			}
		}

		private void PlaceTrench(Map map, IntVec3 position, BuildableDef place)
		{
            ...
            {
                if (GenConstruct.CanBuildOnTerrain(place, position, map, Rot4.North))
                {
                    while (position.GetThingList(map).Count > 0)
                    {
                        position.GetThingList(map)[0].Destroy();
                    }
                    RoadDefGenStep_DryWithFallback.PlaceWorker(map, position, TerrainDefOf.Soil);
                    GenSpawn.Spawn(ThingMaker.MakeThing((ThingDef)place), position, map);
                }
            }
            else
            {
                Log.ErrorOnce($"Can't figure out how to place object {place} while building trench", 10785584);
            }
        }
